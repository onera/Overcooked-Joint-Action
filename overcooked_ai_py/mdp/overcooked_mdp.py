...
class Recipe:
    ...
    def __init__(self, ingredients):
        ...
        self.missing_ingredients_in_pots = {}
        self.cost_to_complete = {}
...
class OvercookedState(object):
    ...
    def clear_order(self, recipe):
        if recipe in self.bonus_orders:
            self._bonus_orders.remove(recipe)
        if recipe in self.all_orders:
            self._all_orders.remove(recipe)
...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_bonus_orders=[], rew_shaping_params=None, layout_name="unnamed_layout", start_all_orders=[], num_items_for_soup=3, order_bonus=2, start_state=None, intentions_sharing = False, **kwargs):
        """
        ...
        self.intentions_sharing = intentions_sharing
        self._opt_recipe_discount_cache = {}
        self._opt_recipe_cache = {}
        self._prev_potential_params = {}
        self.counter_goals=kwargs.get("counter_goals", [])
        self.onion_value = kwargs.get("onion_value", 21)
    ...
    @staticmethod
    def from_layout_name(layout_name, layouts_dir = LAYOUTS_DIR,**params_to_overwrite):
        """
        ...
        base_layout_params = read_layout_dict(layout_name, layouts_dir)
    ...
    @staticmethod
    def from_grid(layout_grid, base_layout_params={"counter_goals":[]}, params_to_overwrite={}, debug=False):
        """
        ...
        for y, row in enumerate(layout_grid):
            for x, c in enumerate(row):
                ...
                if c == "Y":
                    layout_grid[y][x] = 'X'
                    mdp_config["counter_goals"].append((x,y))
    ...
    def mdp_params(self):
        return {
            ...
            "rew_shaping_params": copy.deepcopy(self.reward_shaping_params),
            "start_all_orders" : self.start_all_orders,
            "intentions_sharing" : self.intentions_sharing
        }

    def to_layout_file(self, filepath):
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write("{\n")
            f.write("\t\"grid\":  \"\"\"")
            for i, l in enumerate(self.terrain_mtx):
                for j, c in enumerate(l):
                    if (j,i) == self.start_player_positions[0]:
                        f.write('1')
                    elif (j,i) == self.start_player_positions[1]:
                        f.write('2')
                    else : 
                        f.write(c)
                if i<len(self.terrain_mtx)-1:
                    f.write("\n\t\t\t\t")
            f.write("\"\"\",\n")
            f.write("\t\"start_bonus_order\": " + str(self.start_bonus_orders) + ",\n")
            f.write("\t\"start_all_orders\": " +str(self.start_all_orders) + ",\n")
            f.write("\t\"rew_shaping_params\": None,\n")
            f.write("\t\"counter_goals\":" +str(self.counter_goals) +",\n")
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                if player.has_object() and not new_state.has_object(i_pos):
                    ...
                    if i_pos in self.counter_goals:
                        events_infos[obj_name+'_exchange'][player_idx] = True 
