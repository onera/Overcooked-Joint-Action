...
COUNTERS_MLG_PARAMS = {
        'start_orientations': False,
        'wait_allowed': False,
        'counter_goals': [(0,2), (0,4), (6,2), (6,4), (2,6), (4,6), (2,0), (4,0)],
        'counter_drop': [(0,2), (0,4), (6,2), (6,4), (2,6), (4,6), (2,0), (4,0)],
        'counter_pickup': [],
        'same_motion_goals': True
}
...
class MotionPlanner(object):
    ...
    def __init__(self, mdp, counter_goals=[]):
        ...
        self.costs_dict = self._calculate_heuristic_costs()
        pass
    ...
    def from_pickle_or_compute(mdp, counter_goals, custom_filename=None, force_compute=False, info=False):
        ...
        try:
            ...
            if mp.counter_goals != counter_goals or mp.mdp.terrain_mtx != mdp.terrain_mtx:
                if info:
    ...
    def _calculate_heuristic_costs(self):
        ...
        heuristic_cost_dict = {
            'pot-delivery': self.min_cost_between_features(pot_locations, delivery_locations, manhattan_if_fail=False),
            'dish-pot': self.min_cost_between_features(dish_locations, pot_locations, manhattan_if_fail=False)
        }

        onion_pot_cost = self.min_cost_between_features(onion_locations, pot_locations, manhattan_if_fail=False)
        tomato_pot_cost = self.min_cost_between_features(tomato_locations, pot_locations, manhattan_if_fail=False)
        ...
        if onion_delivery_cost != np.inf :
            heuristic_cost_dict['onion-delivery'] = onion_delivery_cost
        if onion_delivery_cost != np.inf :
            heuristic_cost_dict['tomato-delivery'] = tomato_delivery_cost
...
class MediumLevelActionManager(object):
    ...
    @staticmethod
    def compute_mlam(filename, mdp, mlam_params, info=True):
        final_filepath = os.path.join(PLANNERS_DIR, filename)
        if info:
